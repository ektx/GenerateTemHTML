...
function generateTemHTML(temlpate, data) {
	...
	var generateTem = function(temNo, JSON) {
		...
		if (typeof JSON == 'object' || typeof JSON == 'string') {
			for (var m = 0; m < matchLen; m++) {
				...
				if (arrVal.indexOf('|') > 0) {
					// 取默认值
					defVal = arrVal.split(' | ')[1].replace(/'/g, '');
					arrVal = arrVal.split(' | ')[0];
				}
				...
				if (typeof JSON == 'string') {
					...
					// 取值
					dataVal = JSON[arrVal];
					dataVal = dataVal ? dataVal : defVal ? defVal: '<无>'
	...
	var createTemObj = function(ele) {
		...
		var createTem = function(elelment) {
			...
			if (elelment.find('[tem-type="child-template"]').size() > 0) {
				...
				obj['___tem'+_index+'_'] = removeHUI(html);
				_index++;
	...
	*/
	...
	// 移除模板中的 hui- 索引头
	var removeHUI = function(html) {
		return html = html.replace(/\shui-/gi, ' ')
	}
